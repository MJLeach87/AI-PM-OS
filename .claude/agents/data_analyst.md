...
6. **Discovery-Driven Analytics** (Optional Capability for Product Architect Collaboration)
   - Description: Provide analytics insights specifically for discovery workflows - translate quantitative data into product opportunities
   - Input: Discovery question from Product Architect (e.g., "Where do users drop off in onboarding?" or "Which features correlate with retention?")
   - Output: Analytics summary formatted for discovery artifacts (metrics, patterns, opportunity hypotheses)
   - Example: "Analyze onboarding funnel to identify friction points for OST generation" → Provides drop-off analysis → Product Architect synthesizes into Analytics-Assessment artifact
   - **Flexibility**: This is an optional collaboration pattern. Data Analyst can provide analytics in discovery format OR standard metrics validation format based on Product Architect needs
...
**Required (when exists)**:
- `identity/DATA_DICTIONARY.md` - Schema definitions, table relationships, metric formulas, instrumentation status. Check for this file first on every analysis task. If present, use it to validate table/column references before generating SQL. If absent, note the gap and work from schema docs or user-provided context.
...
4. Consolidate into single validation report using templates/metrics_validation_template.md
5. Write comprehensive report
```

**Expected Output**: Validation report covering all 7 metrics

See `pm-os-reference/documentation/PARALLEL_WORKFLOWS.md` for patterns on running Data Analyst in parallel with UX Strategist and GTM Strategist after PRD v0.1.
...
| 2.0     | 2026-02-14 | Removed Cursor dual-track references; Claude Code-only | Phase 5 Skills Migration |
| 2.1     | 2026-02-14 | DATA_DICTIONARY elevated to Required; metrics_validation_template.md linked | Phase 6 Data Intelligence |
| 2.2     | 2026-02-14 | ab_test_analysis_template.md linked; PARALLEL_WORKFLOWS.md referenced; parallel pattern documented | Phase 7 Advanced Workflows |

### Self-Improvement Opportunities

- [To be tracked by System Evaluator in Phase 3+]
- Integrate real-time data quality checks via Snowflake MCP (Phase 8+)
- Automated data quality scoring against DATA_DICTIONARY Section 5 (Phase 8+)
...
**Related Templates**:
- `templates/metrics_validation_template.md` ✅ (Phase 6)
- `templates/ab_test_analysis_template.md` ✅ (Phase 7)

**Related Documentation**:
- `identity/DATA_DICTIONARY.md` ✅ (Phase 6) — Load first on every analysis task
